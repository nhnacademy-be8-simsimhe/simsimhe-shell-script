...
nohup java -jar -Dserver.port=${port} -DLOG_N_CRASH_APP_KEY=${LOG_N_CRASH_APP_KEY}  ~/target/gateway-0.0.1-SNAPSHOT.jar &
sleep 5
...
if [ $PORT_HEALTH -eq 1 ];
then
  echo -e "$ip:$port에 정상적으로 spring cloud gateway server가 실행 중입니다."
else
  echo -e "$ip:$port에 정상적으로 spring cloud gateway server가 실행 중이 아닙니다."
  exit 0
fi
